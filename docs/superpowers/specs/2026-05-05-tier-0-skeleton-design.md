...
- **D8 — ArduinoFake position:** We do not ship `fabiobatsilva/ArduinoFake` as a `lib_deps` entry. Our behavioral fakes are the recommended path. Users who want call-recording mocks can add ArduinoFake themselves; we do not test or support the combination. Documentation explains the conceptual difference so users can make an informed choice. See [ADR-0003](../../decisions/0003-supersede-arduinofake-coexistence.md) (supersedes [ADR-0002](../../decisions/0002-arduinofake-coexistence.md)).
